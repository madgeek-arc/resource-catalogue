...
public class OIDCSecurityService implements SecurityService {
    ...
    @Override
    public <T extends gr.uoa.di.madgik.domain.Service> boolean isResourceProviderAdmin(Authentication auth, T service) {
        if (hasRole(auth, "ROLE_ANONYMOUS")) {
            return false;
        }
        User user = User.of(auth);
    ...
    @Override
    public <T extends gr.uoa.di.madgik.domain.Service> boolean providerCanAddResources(Authentication auth, T service) {
