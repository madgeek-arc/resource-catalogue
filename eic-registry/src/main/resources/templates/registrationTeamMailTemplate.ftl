...
A new application by [${user.fullName}] – [${user.email}] has been received for registering [${service.name}] - [${service.id}], as a new Resource of [${providerBundle.provider.name}] - ([${providerBundle.provider.id}]).
You can review the application here ${endpoint}/provider/all and approve or reject it.
</#if>
...
The service: [${service.id}] provided by [${user.fullName}] – [${user.email}] has been rejected.
