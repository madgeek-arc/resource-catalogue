...
<#if providerBundle.templateStatus == "no template status">
    ...
    <#if providerBundle.status == "approved provider">
        ...
        <#else>
            The ${serviceOrDatasource} Provider [${providerBundle.provider.name}] - ([${providerBundle.provider.id}]) has been set to inactive.
            You can view the application status here ${endpoint}/dashboard/${providerBundle.provider.id}/info.
        </#if>
    </#if>
...
<#else>
    ...
    A new application by [${user.fullName}] – [${user.email}] has been received for registering [${resource.name}] - [${resource.id}], as a new ${serviceOrDatasource} of [${providerBundle.provider.name}] - ([${providerBundle.provider.id}]).
    You can review the application here ${endpoint}/dashboard/${providerBundle.provider.id}/info and approve or reject it.
    </#if>
    ...
    The ${serviceOrDatasource}: [${resource.id}] provided by [${user.fullName}] – [${user.email}] has been rejected.
