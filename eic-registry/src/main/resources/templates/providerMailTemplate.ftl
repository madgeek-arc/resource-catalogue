...
You have applied for registering your organization [${provider.payload.id}] - [${provider.payload.name}] as a new service provider in eInfraCentral.
...
The information for the service [${service.id}] – [${service.name}] has been reviewed and has been successfully added to the eInfraCentral Catalogue. You can view the new service ${endpoint}/service/${service.id}. You may now login and proceed with providing more services for [${provider.payload.id}].
    <#else>
Your service provider [${provider.payload.name}] has been set to inactive. For any further clarifications please contact registration office of eInfraCentral at regitration@eInfraCentral.eu.
    </#if>
</#if>
