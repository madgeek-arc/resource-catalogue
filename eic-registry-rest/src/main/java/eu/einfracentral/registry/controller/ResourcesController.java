...
@Api(value = "Get Information about a Resource")
public class ResourcesController {
    ...
    @Autowired
    ResourcesController(InfraServiceService<InfraService, InfraService> service,
    ...
    public ResponseEntity<Service> getResource(@PathVariable("id") String id, @ApiIgnore Authentication auth) {
    ...
    public ResponseEntity<?> getResource(@PathVariable("id") String id, @PathVariable("version") String version,
        ...
        if ("rich".equals(id)) { // wrong controller (id = rich, version = serviceId)
            return getRichResource(version, "latest", auth);
        }
        return new ResponseEntity<>(infraService.get(id, version).getService(), HttpStatus.OK);
    }

    // Get the specified version of a RichResource providing the Resource id and version.
    ...
    public ResponseEntity<RichService> getRichResource(@PathVariable("id") String id, @PathVariable("version") String version,
                                                      @ApiIgnore Authentication auth) {
    ...
    public ResponseEntity<Service> addResource(@RequestBody Service resource, @ApiIgnore Authentication auth) {
        InfraService ret = this.infraService.addService(new InfraService(resource), auth);
        logger.info("User '{}' created a new Resource with name '{}' and id '{}'", auth.getName(), resource.getName(), resource.getId());
    ...
    @Deprecated
    public ResponseEntity<Service> resourceWithKPIs(@RequestBody Map<String, JsonNode> json, @ApiIgnore Authentication auth) throws ResourceNotFoundException {
        ...
        InfraService s = null;
        try { // check if resource already exists
            if (service.getId() == null || "".equals(service.getId())) { // if resource id is not given, create it
            ...
            // continue with the creation of the resource
        }

        if (s == null) { // if existing resource is null, create it, else update it
            s = this.infraService.addService(new InfraService(service), auth);
            logger.info("User '{}' added Resource:\n{}", auth.getName(), infraService);
        } else {
            s.setService(service); // replace with given Resource
    ...
    public ResponseEntity<Service> updateResource(@RequestBody Service resource, @ApiIgnore Authentication auth) throws ResourceNotFoundException {
        InfraService ret = this.infraService.updateService(new InfraService(resource), auth);
        logger.info("User '{}' updated Resource with name '{}' and id '{}'", auth.getName(), resource.getName(), resource.getId());
    ...
    public ResponseEntity<Boolean> validate(@RequestBody Service resource) {
        ResponseEntity<Boolean> ret = ResponseEntity.ok(infraService.validate(new InfraService(resource)));
        logger.info("Validated Resource with name '{}' and id '{}'", resource.getName(), resource.getId());
        return ret;
    }
    ...
    public ResponseEntity<Paging<Service>> getAllResources(@ApiIgnore @RequestParam MultiValueMap<String, Object> allRequestParams, @ApiIgnore Authentication authentication) {
    ...
    public ResponseEntity<Paging<RichService>> getRichResources(@ApiIgnore @RequestParam MultiValueMap<String, Object> allRequestParams, @ApiIgnore Authentication auth) {
        FacetFilter ff = FacetFilterUtils.createMultiFacetFilter(allRequestParams);
        ff.addFilter("active", true);
        ff.addFilter("latest", true);
    ...
    public ResponseEntity<List<Service>> getSomeResources(@PathVariable("ids") String[] ids, @ApiIgnore Authentication auth) {
    ...
    public ResponseEntity<List<RichService>> getSomeRichResources(@PathVariable String[] ids, @ApiIgnore Authentication auth) {
...
//    @PreAuthorize("hasRole('ROLE_ADMIN') or @securityService.isProviderAdmin(#auth,#id)")
    public ResponseEntity<Paging<InfraService>> getResourcesByProvider(@ApiIgnore @RequestParam MultiValueMap<String, Object> allRequestParams, @RequestParam(required = false) Boolean active, @PathVariable String id, @ApiIgnore Authentication auth) {
        FacetFilter ff = FacetFilterUtils.createMultiFacetFilter(allRequestParams);
        ff.addFilter("latest", true);
    ...
    // Get all modification details of a specific Resource, providing the Resource id.
    ...
    // Get all modifications of a specific Resource, providing the Resource id and the core-resource Version id.
    @GetMapping(path = {"history/{resourceId}/{versionId}"}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<Service> getVersionHistory(@PathVariable String resourceId, @PathVariable String versionId, @ApiIgnore Authentication auth) {
        Service service = infraService.getVersionHistory(resourceId, versionId);
        return ResponseEntity.ok(service);
    }

    // Get all featured Resources.
    @GetMapping(path = "featured/all", produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<List<Service>> getFeaturedResources() {
        return new ResponseEntity<>(infraService.createFeaturedServices(), HttpStatus.OK);
    }

    // Filter a list of inactive Resources based on a set of filters or get a list of all inactive Resources in the Catalogue.
    @ApiImplicitParams({
            @ApiImplicitParam(name = "query", value = "Keyword to refine the search", dataType = "string", paramType = "query"),
            @ApiImplicitParam(name = "from", value = "Starting index in the result set", dataType = "string", paramType = "query"),
            @ApiImplicitParam(name = "quantity", value = "Quantity to be fetched", dataType = "string", paramType = "query"),
    ...
    public ResponseEntity<Paging<Service>> getInactiveResources(@ApiIgnore @RequestParam MultiValueMap<String, Object> allRequestParams, @ApiIgnore Authentication auth) throws ResourceNotFoundException {
        FacetFilter ff = FacetFilterUtils.createMultiFacetFilter(allRequestParams);
...
//        Paging<InfraService> infraServices = infraService.getInactiveServices();
    ...
    // Providing the Resource id and version, set the Resource to active or inactive.
    ...
    // Get all pending Resource Templates.
